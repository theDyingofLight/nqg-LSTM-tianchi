...
class BeamSearcher(object):
    ...
    def decode(self):
        ...
        for i, eval_data in enumerate(self.data_loader):
            src_seq, src_mask, trg_seq, trg_mask, tag_seq= eval_data

            best_question = self.beam_search(src_seq, src_mask,tag_seq)
            # discard START  token
            ...
            decoded_words = self.tokenizer.convert_tokens_to_string(decoded_words)
            golden_question = self.test_data[i]
            print("write {}th question\r".format(i))
            final=''.join(decoded_words.split(' '))
            pred_fw.write(final + "\n")
    ...
    def beam_search(self, src_seq, src_mask,tag_seq):
        ...
        while num_steps < config.max_decode_step and len(results) < config.beam_size:
            ...
            # [beam_size, |V|]
            logits, states, context_vector = self.model.decoder.decode(prev_y, src_seq,
                                                                       prev_states, prev_context,
                                                                       enc_features, src_mask)
