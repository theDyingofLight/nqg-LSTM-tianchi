...
class Decoder(nn.Module):
    ...
    def forward(self, trg_seq,ext_src_seq,init_states, encoder_outputs, src_mask):
        # trg_seq : [b,t]
        # init_states : [2,b,d]
        ...
        for i in range(max_len):
            ...
            context, energy = self.attention(output, memories, src_mask)
...
class Seq2seq(nn.Module):
    def __init__(self, embedding=None):
        ...
        self.embedding = self.bert_encoder.bert.get_input_embeddings()
        self.vocab_size = self.bert_encoder.model_config.vocab_size
        self.encoder = Encoder(self.embedding,
                               ...
                               config.num_layers,
                               config.dropout)
        self.decoder = Decoder(embedding, self.vocab_size,
                               config.embedding_size,
                               2 * config.hidden_size,
                               config.num_layers,
                               config.dropout)

    def forward(self, src_seq,src_mask,tag_seq, trg_seq,trg_mask):
        src_len = torch.sum(src_mask, 1)
        enc_outputs, enc_states = self.encoder(src_seq, src_len,tag_seq)
        sos_trg = trg_seq[:, :-1].contiguous()


        logits = self.decoder(sos_trg, src_seq,
                              enc_states, enc_outputs, src_mask)
        return logits
